...
const HAS_SUPABASE=Boolean(window.supabase&&CFG.url&&CFG.anonKey&&!String(CFG.url).includes('YOUR_'));
...
function slideshow(root,items){
 ...
 dots.innerHTML=items.map((_,n)=>`<button class="dot ${n===0?'active':''}" aria-label="Go to media ${n+1}"></button>`).join('');
 const go=n=>{i=(n+items.length)%items.length;slides.querySelectorAll('.slide').forEach((s,j)=>{s.classList.toggle('active',j===i);if(j!==i){const v=s.querySelector('video');if(v)v.pause()}});dots.querySelectorAll('.dot').forEach((d,j)=>d.classList.toggle('active',j===i));if(counter)counter.textContent=`${String(i+1).padStart(2,'0')} / ${String(items.length).padStart(2,'0')}`};
 root.querySelector('.prev').onclick=()=>go(i-1);root.querySelector('.next').onclick=()=>go(i+1);dots.querySelectorAll('.dot').forEach((d,n)=>d.onclick=()=>go(n));
...
function booking(){
 ...
 const reset=()=>{view.hidden=false;previewView.hidden=true;form.reset()};document.getElementById('editBooking').onclick=reset;
 form.onsubmit=async e=>{e.preventDefault();const f=new FormData(form),name=String(f.get('name')).trim(),phone=String(f.get('phone')).trim(),service=String(f.get('service')),date=String(f.get('date')),time=String(f.get('time')),notes=String(f.get('notes')||'').trim();
 const text=`Crosses & Clippers Appointment\n\nName: ${name}\nPhone: ${phone}\nService: ${service}\nDate: ${date}\nTime: ${time}\nNotes: ${notes||'None'}`;
 preview.textContent=text;wa.href=`https://wa.me/${WA}?text=${encodeURIComponent(text)}`;mail.href=`mailto:${EMAIL}?subject=${encodeURIComponent('Crosses & Clippers Appointment')}&body=${encodeURIComponent(text)}`;
 const row={name,phone,service,date,time,notes,status:'new',created_at:new Date().toISOString()};
 try{if(sb){await sb.from('appointments').insert(row);await sb.from('clients').insert({name,phone,notes})}else{const cs=localGet(STORE.clients,[]);cs.unshift({...row,id:crypto.randomUUID()});localSet(STORE.clients,cs);const ap=localGet(STORE.appointments,[]);ap.unshift({...row,id:crypto.randomUUID()});localSet(STORE.appointments,ap)}}catch(err){console.warn(err)}
...
async function renderReviews(){
 const rs=await fetchApprovedReviews(),list=document.getElementById('reviewList');document.getElementById('reviewCount').textContent=rs.length;const avg=rs.length?rs.reduce((a,r)=>a+Number(r.rating),0)/rs.length:0;document.getElementById('avgRating').textContent=avg.toFixed(1);document.getElementById('ratingStars').textContent=stars(Math.round(avg));
 list.innerHTML=rs.length?rs.map(r=>`<article class="review-item"><div class="stars">${stars(r.rating)}</div><h4>${esc(r.name)}</h4><p>${esc(r.comment)}</p><small>${new Date(r.created_at||r.createdAt).toLocaleDateString('en-ZA',{day:'2-digit',month:'short',year:'numeric'})}</small></article>`).join(''):'<div class="empty-state">No published reviews yet. Be the first to share your experience.</div>';
}
...
async function admin(){
 ...
 form.onsubmit=async e=>{e.preventDefault();const f=new FormData(form),email=String(f.get('email')).trim(),password=String(f.get('password'));try{if(sb){const {error}=await sb.auth.signInWithPassword({email,password});if(error)throw error}else{const saved=localGet(STORE.owner,null);if(saved&&!saved.email===email)throw new Error('Invalid owner login');if(!saved)localSet(STORE.owner,{email,password});else if(saved.email!==email||saved.password!==password)throw new Error('Invalid owner login')}loginView.hidden=true;panel.hidden=false;await showAdmin();}catch(err){document.getElementById('adminStatus').textContent=err.message||'Login failed.'}}
 const session=await getOwnerSession();if(session){loginView.hidden=true;panel.hidden=false;showAdmin()}
 async function showAdmin(){
  let reviewsData=[],clients=[],appointments=[];
  if(sb){const r=await sb.from('reviews').select('*').order('created_at',{ascending:false});reviewsData=r.data||[];const c=await sb.from('clients').select('*').order('created_at',{ascending:false});clients=c.data||[];const a=await sb.from('appointments').select('*').order('created_at',{ascending:false});appointments=a.data||[]}else{reviewsData=localGet(STORE.reviews,[]);clients=localGet(STORE.clients,[]);appointments=localGet(STORE.appointments,[])}
  ...
  const renderTab=tab=>{panel.querySelectorAll('.admin-tabs button').forEach(b=>b.classList.toggle('active',b.dataset.tab===tab));
   if(tab==='reviews')content.innerHTML=reviewsData.length?reviewsData.map(r=>`<div class="admin-review"><div><strong>${esc(r.name)} • ${stars(r.rating)}</strong> <span class="status-pill">${esc(r.status|| (r.approved?'approved':'pending'))}</span></div><p>${esc(r.comment)}</p><div class="admin-actions">${r.status!=='approved'?`<button class="approve" data-approve="${r.id}">APPROVE</button>`:''}${r.status!=='rejected'?`<button data-reject="${r.id}">REJECT</button>`:''}<button data-delete-review="${r.id}">DELETE</button></div></div>`).join(''):'<p class="muted">No reviews yet.</p>';
   if(tab==='clients')content.innerHTML=clients.length?clients.map(c=>`<div class="client-row"><strong>${esc(c.name)}</strong><span>${esc(c.phone||c.email||'No contact')}</span><span>${esc(c.email||'')}</span><span>${new Date(c.created_at||c.createdAt).toLocaleDateString('en-ZA')}</span></div>`).join(''):'<p class="muted">No client records yet.</p>';
   if(tab==='appointments')content.innerHTML=appointments.length?appointments.map(a=>`<div class="appointment-row"><strong>${esc(a.name)} • ${esc(a.service)}</strong><span>${esc(a.date)} at ${esc(a.time)}</span><span>${esc(a.phone)}</span><span>${esc(a.notes||'No notes')}</span><small class="muted">${new Date(a.created_at||a.createdAt).toLocaleString('en-ZA')}</small></div>`).join(''):'<p class="muted">No appointments recorded yet.</p>';
  };
  renderTab('reviews');panel.querySelectorAll('[data-tab]').forEach(b=>b.onclick=()=>renderTab(b.dataset.tab));
  panel.querySelectorAll('[data-approve]').forEach(b=>b.onclick=async()=>{await setReviewStatus(b.dataset.approve,'approved');await showAdmin();await renderReviews()});panel.querySelectorAll('[data-reject]').forEach(b=>b.onclick=async()=>{await setReviewStatus(b.dataset.reject,'rejected');await showAdmin()});panel.querySelectorAll('[data-delete-review]').forEach(b=>b.onclick=async()=>{await deleteReview(b.dataset.deleteReview);await showAdmin();await renderReviews()});
 }
